...
const Mobile_buttons = async () => {
  ...
  if (
    !Stick_button ||
    !Stick_button.sticky_call_button ||
    !Stick_button.sticky_mail_button
  ) {
    return null;
  }

  const { sticky_call_button, sticky_mail_button } = Stick_button;

  return (
    <div className="mobile-sticky fixed bottom-0 left-0 w-full z-[99999] sm:hidden block">
      <div className="flex mobile-s-wrapper">
        {/* ✅ Call Button */}
        {sticky_call_button?.url && (
          <Link
            href={sticky_call_button.url}
            target={sticky_call_button.target}
        ...
        {sticky_mail_button?.url && (
          ...
          >
            ...
            <p className="text-Teal">{sticky_mail_button.title}</p>
          </Link>
        )}
      </div>
    </div>
